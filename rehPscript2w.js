...
function drawSnake() {
    if(!hittingWall() && !hittingItself()) {
        ...
        for(var i = snakeArray.length - 1; i >= 1; i--) {
            ...
            snakeArray[1].yPos = snakeArray[i-1].yPos;
            rect(snakeArray[i].xPos, snakeArray[i].yPos, 19, 19);
        }
    ...
    function hittingItself() {
        ...
        var piecesHit = allButHead.filter(snakePiece => snakePiece.xPos == headX &&
        snakePiece.yPos == headY).length;
        return piecesHit;
    }
    ...
    function moveApple() {
        ...
        while(appleOnSnake.length == 1) {
            ...
            appleOnSnake = snakeArray.filter(snakePiece => snakePiece.xPos == randomX
            && snakePiece.xPos == randomY) .length;
        }
        appleX = randomX;
        appleY = randomY;
    }
...
/*
    function hittingApple() {
        ...
        if(headX == appleX && headY == appleY) {
            console.log('hit');
            makeSnakePiece(snakeArray[snakeArray.length - 1].xPos,
            snakeArray[snakeArray.length - 1].yPos);
            score++;
            moveApple();
        }
    }
